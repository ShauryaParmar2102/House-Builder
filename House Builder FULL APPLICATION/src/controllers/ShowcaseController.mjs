...
import { PricingCatalogueModel } from "../models/PricingCatalogueModel.mjs"; //Imports PricingCatalogueModel so that users can use it to get pricing data and calculate house costs
import { CompanyModel } from "../models/CompanyModel.mjs"; //Imports company related logic like getting all building companies and validating if a company exists
import { ShowcasePostModel } from "../models/ShowcasePostModel.mjs"; //Imports the model that manages showcase builds like saving builds, fetching builds, deleting builds and updating builds
 ...
 * - Creates new showcase builds
 * - Validates user input
 * - Calculates total cost of builds
 * - Supports updating and deleting builds
 */
export class ShowcaseController {

  /**
 * Retrieves all showcase builds
 * @returns {Array} list of showcase builds
 */
  ...
  static async getShowcaseJSON(req, res) { //Used for creating a new house build using form data.
    ...
    /**
    * Creates a new showcase build
    * - Validates user input
    * - Calculates total cost
    * - Saves build to storage
    * @param {Object} req
    * @param {Object} res
    */

  static async publishBuild(req, res) { //Main function that creates a house build (POST)
    ...
    try {
      ...
      // Builds house 
      const build = {
        id: house.id ||`sc_${Date.now()}`, // unique ID
  ...
  /**
  * Updates an existing showcase build
  * - Validates input
  * - Recalculates total cost
  * - Updates stored record
  * @param {Object} req
  * @param {Object} res
  */
  ...
  static async updateBuild(req, res) { //Defines an API function for updating a house
    ...
    try { //Wraps code in safety net. If something breaks go to catch
      ...
      house.extras?.forEach(extraName => { //Calculates Extras Pricing
        const extraData = pricing.extras.find(e => e.name === extraName); //Matches user selection to pricing catalogue
        if (extraData) totalCost += extraData.price; //Adds extra cost only if valid
      });
  ...
  /**
  * Deletes a showcase build by ID
  * @param {Object} req
  * @param {Object} res
  */
